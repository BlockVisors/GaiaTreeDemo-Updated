...
contract Gaiatrees is AccessControl, DetailedERC721 {
    ...
    Gaiatree[] private gaiatrees;

    uint256 private startingPrice = 0.04 ether;
    ...
    function getToken(uint256 _tokenId) public view returns (
        ...
        _tokenName = gaiatrees[_tokenId].name;
        _dna = gaiatrees[_tokenId].dna;
    ...
    function purchase(uint256 _tokenId) public payable whenNotPaused {
        ...
        TokenSold(
            _tokenId,
            gaiatrees[_tokenId].name,
            gaiatrees[_tokenId].dna,
            sellingPrice,
            priceOf(_tokenId),
            oldOwner,
            newOwner
    ...
    function _createToken(string _name, bytes5 _dna, address _owner, uint256 _price) private {
        Gaiatree memory _gaiatree = Gaiatree({
            name: _name,
            dna: _dna
        });
        uint256 newTokenId = gaiatrees.push(_gaiatree) - 1;
